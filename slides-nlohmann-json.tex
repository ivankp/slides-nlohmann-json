...
\definecolor{lstPreproc}{HTML}{98BF10} % F06090
...
@Tbasic_json@::@Fparse@(); @!{\hfill
    ...
    \begin{itemize}
      \item \begin{lstlisting}
@Tjson@ j = @Tjson@::@Fparse@(std::@Tifstream@("/opt/data.json"));
        \end{lstlisting}

      \item \begin{lstlisting}
@Tjson@ j = @Tjson@::@Fparse@(R"({ "key": "value" })");
        \end{lstlisting}
    \end{itemize}

  \item \begin{lstlisting}
std::@Tistream@&
operator>>(std::@Tistream@&, @Tbasic_json@&); @!\hfill
...
@Tbasic_json@::@Faccept@(); @!\hfill
  ...
  \item \begin{lstlisting}
@Tjson@
operator @L""_json@(const char*, std::size_t); @!\hfill
...
std::@Tstring@ str = j.@Fdump()@;
  ...
  \item \begin{lstlisting}
std::@Tistream@&
operator<<(std::@Tistream@&, const @Tbasic_json@&); @!\hfill
      \href{https://json.nlohmann.me/api/operator_ltlt/}{\faFileTextO}@
    \end{lstlisting}
    \begin{itemize}
      \item \begin{lstlisting}
@Tjson@ j;
std::@Tofstream@("/opt/data.json") << j;
        \end{lstlisting}
...
\begin{lstlisting}
const @Tjson@ j = R"(@!\color{lstNamespace}\{@ "a"@!\color{lstNamespace}:@ "value" @!\color{lstNamespace}\}@)"@L_json@;
\end{lstlisting}
...
const @Tjson@ j = R"(@!\color{lstNamespace}\{@ "a"@!\color{lstNamespace}:@ @!\color{lstNamespace}\{@ "b"@!\color{lstNamespace}:@ 1 @!\color{lstNamespace}\}@ @!\color{lstNamespace}\}@)"@L_json@;
\end{lstlisting}
...
\begin{itemize}[itemsep=0.6em]
  \item \lstinline{operator[]()} models mutable access
    \begin{itemize}
...
\hline
\lstinline{operator[]()} & undefined behavior & insert & insert \hfill
...
\lstinline{operator[]() const} & undefined behavior & doesn't exist & throw \\
...
@TValue@& @Tbasic_json@::@Fget_to@(@TValue@&) const @!\hfill
  ...
  \item \begin{lstlisting}
template <typename @TValue@>
@TValue@ @Tbasic_json@::@Fget_ref@() @!\itshape\color{lstKeyword}const@ @!\hfill
